...
保留 per-user '正在评分' 标记（每个用户开始处理前设置）。

通过检测发现问题
1.  控制台没有看到 [慢速评分-完成]  信息。 这个信息 应该在 [中转检测-tx]  后面 发出来， 我可以看到 改变 ，另外 在  Step 3.5: 立即评分  之前 和  之后 都打印一遍 我看 一下 问题。
2. 还是 老问题。 有些账户很快 就给了 散户状态  不知道什么原因 ，后来 通过 [中转检测-tx]  改为 庄家 ，散户状态 才消失。
...
通过测试 发现问题
1.[Step3.5-前] traderStats:    用户状态 都是散户 ，[Step3.5-后] traderStats:   用户状态 区分为 庄家 普通 ，[慢速评分-完成] traderStats 完整快照 用户 有了分数，状态又全变成散户。
2.EXECUTE_TRADES_REFRESH EXECUTE_HOLDERS_REFRESH 获取数据后，对 traderStats 初始化 status 改为 普通。 这个问题就可以解决了 [Step3.5-前] traderStats  用户状态 都是散户:  
3.[慢速评分-完成] traderStats 完整快照 用户 有了分数，状态又全变成散户。 为什么 又 全变回去了。 这里 不是应该 首先 只去处理 普通类型， 然后 每处理完一个 日志显示 traderStats  处理状态， ui  更新 用户状态。 这部分 代码 检查一下 看什么原因怎么修改


_scheduleQuickScore   是否确认  散户   快评分 calculateScores
_scheduleSlowScore    只对 普通 用户 处理 慢评分  detectHiddenRelays

filterUsersByScore 是 score < threshold 的逻辑，但此时应用 status === '散户'。
修复：最终 filteredUsers 改为 status === '散户' 过滤。

问题3：detectHiddenRelays 只处理'普通'用户 + 每用户后打印日志
修复：
unchecked 过滤器新增条件 ⓪：status === '庄家' 直接跳过
每个用户处理完后加 console.log('[中转检测-进度]') 打印当前所有用户状态

_initTrader()           → status = '普通'
updateUserInfo()        → status 不变（排除了 API 的 status 字段）
updateHolderData()
  └─ enable_hidden_relay=true  → '庄家'/'普通'（不写'散户'）
  └─ enable_hidden_relay=false → scoreData.status（散户/庄家，正确）
[Step3.5-前]            → 全部显示'普通'或'庄家' ✓
Step 3.5                → 评分，设置'庄家'/'普通'
_scheduleSlowScore()    → detectHiddenRelays 逐个确认 → 散户/庄家
[慢速评分-完成]         → 最终快照，正确区分散户/庄家
